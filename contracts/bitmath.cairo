...
namespace BitMath {
    func _msb_shift{range_check_ptr}(x: Uint256, r: felt, mask: Uint256, bit: felt) -> (
        x: Uint256, r: felt
    ) {
    ...
    func most_significant_bit{range_check_ptr}(num: Uint256) -> (res: felt) {
        alloc_locals;
    ...
    func _lsb_shift{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(
        x: Uint256, r: felt, mask: Uint256, bit: felt
    ) -> (x: Uint256, r: felt) {
        alloc_locals;
    ...
    func least_significant_bit{range_check_ptr, bitwise_ptr: BitwiseBuiltin*}(num: Uint256) -> (
        res: felt
    ) {
        let (is_valid) = uint256_lt(Uint256(0, 0), num);
        assert is_valid = TRUE;
